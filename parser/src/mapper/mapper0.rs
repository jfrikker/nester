...
use super::{add_ram, add_rom, Mapper};
...
impl<'a> Mapper0<'a> {
    pub fn read(contents: &'a [u8]) -> IResult<&'a [u8], Self> {
        ...
        Ok((contents, Self { prg_rom, chr_rom }))
    }
    ...
    fn add_read_mem_decl<'ctx>(&self, context: &'ctx Context, module: &Module<'ctx>) {
        ...
        let ptr = unsafe {
            builder.build_gep(
                module.get_global("lowMem").unwrap().as_pointer_value(),
                &[i16_ty.const_zero(), addr2],
                "ptr",
            )
        };
        ...
        let ptr = unsafe {
            builder.build_gep(
                module.get_global("highMem").unwrap().as_pointer_value(),
                &[i16_ty.const_zero(), addr2],
                "ptr",
            )
        };
        ...
        let ptr = unsafe {
            builder.build_gep(
                module.get_global("prgRom").unwrap().as_pointer_value(),
                &[i16_ty.const_zero(), addr2],
                "ptr",
            )
        };
