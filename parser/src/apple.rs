use inkwell::{module::{Linkage, Module}, context::Context, attributes::AttributeLoc, IntPredicate};
use nom::{IResult, bytes::complete::take};

use crate::{mapper::Mapper, llvm::{INACCESSIBLE_MEM_ONLY, NO_UNWIND, WILL_RETURN}};
...
impl <'a> Apple<'a> {
  pub fn read(contents: &'a [u8]) -> IResult<&'a[u8], Self> {
    let (contents, offset) = nom::number::complete::u16(nom::number::Endianness::Big)(contents)?;
    let (contents, length) = nom::number::complete::u16(nom::number::Endianness::Big)(contents)?;
...
impl <'a> Mapper for Apple<'a> {
    ...
    fn add_globals<'ctx>(&self, context: &'ctx inkwell::context::Context, module: &inkwell::module::Module<'ctx>) {
      let glob = module.add_global(context.i8_type().array_type(0x10000), None, "mem");
      glob.set_initializer(&context.i8_type().const_array(&(0..self.offset).into_iter()
        .map(|_| context.i8_type().const_zero())
        .chain(self.rom.iter().map(|i| context.i8_type().const_int(*i as u64, false)))
        .chain(((self.offset as usize + self.rom.len())..0x10000).into_iter()
          .map(|_| context.i8_type().const_zero()))
        .collect::<Vec<_>>()));
...
fn add_read_mem_decl<'ctx>(context: &'ctx Context, module: &Module<'ctx>) {
  ...
  let cond = builder.build_int_compare(IntPredicate::ULT, addr, i16_ty.const_int(0xd012, false), "cond");
  builder.build_conditional_branch(cond, if_low, else_low);

  builder.position_at_end(if_low);
  let ptr = unsafe {
    builder.build_gep(module.get_global("mem").unwrap().as_pointer_value(), &[i64_ty.const_zero(), addr_ext], "ptr")
  };
  let val = builder.build_load(ptr, "val");
  builder.build_return(Some(&val));

  builder.position_at_end(else_low);
  let cond = builder.build_int_compare(IntPredicate::ULT, addr, i16_ty.const_int(0xd013, false), "cond");
