...
toIR_ inst@(I.AbsoluteX _ I.ROR arg) = absoluteXAddr arg >>= _ror >> brNext inst
...
_ror addr = do
  ...
  store regC 0 newC
  writeMem (LocalReference (ptr writeCallbackType) "writeCallback") addr newVal2
  setNZ newVal2
_store reg addr = do
  val <- load reg 0
