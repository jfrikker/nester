...
pub const INACCESSIBLE_MEM_ONLY: u32 = 9;
// pub const NO_ALIAS: u32 = 16;
// pub const NO_CAPTURE: u32 = 18;
pub const NO_UNWIND: u32 = 30;
// pub const READ_ONLY: u32 = 38;
// pub const SRET: u32 = 55;
pub const WILL_RETURN: u32 = 59;
// pub const ZERO_EXT: u32 = 61;
...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    pub fn new(context: &'ctx Context, module: &'a Module<'ctx>) -> Self {
        ...
        let sp = result
            .module
            .add_global(result.context.i8_type(), None, "sp");
    ...
    fn add_uadd_carry_decl(&self) {
        ...
        self.module
            .add_function("llvm.uadd.with.overflow.i8", fn_type, None);
    }
    ...
    fn add_sadd_carry_decl(&self) {
        ...
        let fn_type = result_type.fn_type(&[i8_ty.into(), i8_ty.into()], false);
        self.module
            .add_function("llvm.sadd.with.overflow.i8", fn_type, None);
    }
    ...
    fn add_usub_carry_decl(&self) {
        ...
        let fn_type = result_type.fn_type(&[i8_ty.into(), i8_ty.into()], false);
        self.module
            .add_function("llvm.usub.with.overflow.i8", fn_type, None);
    }
    ...
    fn add_ssub_carry_decl(&self) {
        ...
        let fn_type = result_type.fn_type(&[i8_ty.into(), i8_ty.into()], false);
        self.module
            .add_function("llvm.ssub.with.overflow.i8", fn_type, None);
    }
    ...
    pub fn declare_func(&mut self, addr: u16) {
        ...
        self.module
            .add_function(&function_name(addr), fn_type, Some(Linkage::Private));
    }

    pub fn define_func(&mut self, addr: u16, body: &BTreeMap<u16, Instruction>) {
        let func = self
            .module
            .get_function(&function_name(addr))
...
fn function_return_type(context: &Context) -> StructType {
    ...
    context.struct_type(
        &[
            i8_ty, i8_ty, i8_ty, i1_ty, i1_ty, i1_ty, i1_ty, i8_ty, i16_ty,
        ],
        false,
    )
}
...
impl<'a, 'b, 'ctx> FunctionCompiler<'a, 'b, 'ctx> {
    fn new(compiler: &'a Compiler<'b, 'ctx>, function: FunctionValue<'ctx>) -> Self {
        ...
        let reg_a = compiler
            .builder
            .build_alloca(compiler.context.i8_type(), "regA");
        let reg_x = compiler
            .builder
            .build_alloca(compiler.context.i8_type(), "regX");
        let reg_y = compiler
            .builder
            .build_alloca(compiler.context.i8_type(), "regY");
        let reg_n = compiler
            .builder
            .build_alloca(compiler.context.bool_type(), "regN");
        let reg_z = compiler
            .builder
            .build_alloca(compiler.context.bool_type(), "regZ");
        let reg_v = compiler
            .builder
            .build_alloca(compiler.context.bool_type(), "regV");
        let reg_c = compiler
            .builder
            .build_alloca(compiler.context.bool_type(), "regC");
        let stack = compiler
            .builder
            .build_alloca(compiler.context.i8_type().array_type(10), "stack");
        let sp = compiler
            .builder
            .build_alloca(compiler.context.i8_type(), "sp");
        compiler
            .builder
            .build_store(reg_a, function.get_nth_param(0).unwrap().into_int_value());
        compiler
            .builder
            .build_store(reg_x, function.get_nth_param(1).unwrap().into_int_value());
        compiler
            .builder
            .build_store(reg_y, function.get_nth_param(2).unwrap().into_int_value());
        compiler
            .builder
            .build_store(reg_n, function.get_nth_param(3).unwrap().into_int_value());
        compiler
            .builder
            .build_store(reg_z, function.get_nth_param(4).unwrap().into_int_value());
        compiler
            .builder
            .build_store(reg_v, function.get_nth_param(5).unwrap().into_int_value());
        compiler
            .builder
            .build_store(reg_c, function.get_nth_param(6).unwrap().into_int_value());
        compiler
            .builder
            .build_store(sp, compiler.context.i8_type().const_zero());
    ...
    fn declare_instruction(&mut self, addr: u16) {
        self.blocks.insert(
            ...
            self.compiler
                .context
                .append_basic_block(self.function, &format!("{:04x}", addr)),
        );
    }
    ...
    fn write_instruction(&self, inst: &Instruction) {
        match inst {
            ...
            Instruction::Absolute {
                ...
                self.builder
                    .build_unconditional_branch(*self.blocks.get(addr).unwrap());
            }
            Instruction::Absolute {
                opcode: Opcode::JSR,
                addr,
            ...
            Instruction::Implied {
                ...
                self.builder
                    .build_store(self.reg_c, self.context.bool_type().const_zero());
            }
            Instruction::Implied {
                opcode: Opcode::CLV,
                ..
                ...
                self.builder
                    .build_store(self.reg_v, self.context.bool_type().const_zero());
            }
            Instruction::Implied {
                opcode: Opcode::DEX,
                ..
            ...
            Instruction::Implied {
                ...
                self.builder
                    .build_store(self.reg_c, self.context.bool_type().const_all_ones());
            }
            Instruction::Implied {
                opcode: Opcode::TAX,
                ..
            ...
            Instruction::Switch {
                ...
                self.builder.build_switch(
                    a,
                    *self.blocks.get(targets.get(0).unwrap()).unwrap(),
                    &targets
                        .iter()
                        .enumerate()
                        .map(|(i, tgt)| {
    ...
    fn absolute_value(&self, addr: u16) -> IntValue<'ctx> {
        ...
        self.builder
            .build_call(
                read_mem,
                &[self.context.i16_type().const_int(addr as u64, false).into()],
                "absVal",
            )
            .try_as_basic_value()
            .unwrap_left()
    ...
    fn set_n(&self, val: IntValue) {
        let n = self.builder.build_int_compare(
            IntPredicate::UGT,
            val,
            self.context.i8_type().const_int(0x7f, false),
            "n",
        );
        self.builder.build_store(self.reg_n, n);
    }
    ...
    fn set_z(&self, val: IntValue) {
        let z = self.builder.build_int_compare(
            IntPredicate::EQ,
            val,
    ...
    fn adc(&self, arg: IntValue<'ctx>) {
        ...
        let c_ext = self
            .builder
            .build_int_z_extend(c, self.context.i8_type(), "c_ext");
        let a = self.builder.build_load(self.reg_a, "a").into_int_value();
    ...
    fn add_carry(
        ...
        let uadd = self
            .compiler
            .module
            .get_function("llvm.uadd.with.overflow.i8")
            .unwrap();
        let sadd = self
            .compiler
            .module
            .get_function("llvm.sadd.with.overflow.i8")
            .unwrap();
        let res_u = self
            .compiler
            .builder
            .build_call(uadd, &[arg1.into(), arg2.into()], "res_u")
            .try_as_basic_value()
            .unwrap_left()
            .into_struct_value();
        let res_s = self
            .compiler
            .builder
            .build_call(sadd, &[arg1.into(), arg2.into()], "res_s")
            .try_as_basic_value()
            .unwrap_left()
            .into_struct_value();
        let res = self
            .compiler
            .builder
            .build_extract_value(res_u, 0, "res")
            .unwrap()
            .into_int_value();
        let c = self
            .compiler
            .builder
            .build_extract_value(res_u, 1, "c")
            .unwrap()
            .into_int_value();
        let v = self
            .compiler
            .builder
            .build_extract_value(res_s, 1, "v")
            .unwrap()
            .into_int_value();
        (res, c, v)
    }
    ...
    fn sub_carry(
        ...
        let usub = self
            .compiler
            .module
            .get_function("llvm.usub.with.overflow.i8")
            .unwrap();
        let ssub = self
            .compiler
            .module
            .get_function("llvm.ssub.with.overflow.i8")
            .unwrap();
        let res_u = self
            .compiler
            .builder
            .build_call(usub, &[arg1.into(), arg2.into()], "res_u")
            .try_as_basic_value()
            .unwrap_left()
            .into_struct_value();
        let res_s = self
            .compiler
            .builder
            .build_call(ssub, &[arg1.into(), arg2.into()], "res_s")
            .try_as_basic_value()
            .unwrap_left()
            .into_struct_value();
        let res = self
    ...
    fn asl(&self, arg: IntValue<'ctx>) -> IntValue<'ctx> {
        let new_c1 = self.builder.build_right_shift(
            arg,
        ...
        let new_c2 = self
            .builder
            .build_int_truncate(new_c1, self.context.bool_type(), "new_c2");
        let new_val = self.builder.build_left_shift(
            arg,
    ...
    fn compare(&self, reg: PointerValue, arg: IntValue) {
        ...
        let new_c = self
            .builder
            .build_int_compare(IntPredicate::UGE, reg_val, arg, "new_c");
    ...
    fn decrement(&self, addr: IntValue<'ctx>) {
        ...
        let val_new =
            self.builder
                .build_int_sub(val, self.context.i8_type().const_int(1, false), "val_new");
    ...
    fn increment(&self, addr: IntValue<'ctx>) {
        ...
        let val_new =
            self.builder
                .build_int_add(val, self.context.i8_type().const_int(1, false), "val_new");
        self.set_n_z(val_new);
        self.write_mem(addr, val_new);
    }
    ...
    fn lsr(&self, val: IntValue<'ctx>) -> IntValue<'ctx> {
        let new_c = self
            .builder
            .build_int_truncate(val, self.context.bool_type(), "new_c");
        self.builder.build_store(self.reg_c, new_c);
        let new_val = self.builder.build_right_shift(
            val,
            self.context.i8_type().const_int(1, false),
            false,
            "new_val",
    ...
    fn push(&self, val: IntValue) {
        ...
        let ptr = unsafe {
            self.builder.build_gep(
                self.stack,
                &[self.context.i8_type().const_zero(), sp],
                "ptr",
            )
        };
        ...
        let new_sp =
            self.builder
                .build_int_add(sp, self.context.i8_type().const_int(1, false), "new_sp");
    ...
    fn pop(&self) -> IntValue {
        ...
        let new_sp =
            self.builder
                .build_int_sub(sp, self.context.i8_type().const_int(1, false), "new_sp");
        let ptr = unsafe {
            self.builder.build_gep(
                self.stack,
                &[self.context.i8_type().const_zero(), new_sp],
                "ptr",
            )
        };
        self.builder.build_store(self.sp, new_sp);
    ...
    fn read_mem(&self, addr: IntValue<'ctx>) -> IntValue<'ctx> {
        let read_mem = self.compiler.module.get_function("readMem").unwrap();
        self.builder
            .build_call(read_mem, &[addr.into()], "res")
            .try_as_basic_value()
            .unwrap_left()
            .into_int_value()
    }
    ...
    fn rol(&self, val: IntValue<'ctx>) -> IntValue<'ctx> {
        ...
        let c_ext = self
            .builder
            .build_int_s_extend(c, self.context.i8_type(), "c_ext");
        let new_val1 = self.builder.build_left_shift(
            val,
            self.context.i8_type().const_int(1, false),
            "new_val1",
        );
    ...
    fn ror(&self, val: IntValue<'ctx>) -> IntValue<'ctx> {
        ...
        let c_ext = self
            .builder
            .build_int_s_extend(c, self.context.i8_type(), "c_ext");
        let c_ext2 = self.builder.build_and(
            c_ext,
            self.context.i8_type().const_int(0x80, false),
            "c_ext2",
        );
        let new_val1 = self.builder.build_right_shift(
            val,
            self.context.i8_type().const_int(1, false),
            false,
            "new_val1",
    ...
    fn write_mem(&self, addr: IntValue, val: IntValue) {
        ...
        self.builder
            .build_call(write_mem, &[addr.into(), val.into()], "res");
    }
    ...
    fn set_first_instruction(&self, addr: u16) {
        self.builder
            .position_at_end(self.function.get_first_basic_block().unwrap());
        self.builder
            .build_unconditional_branch(*self.blocks.get(&addr).unwrap());
    }
    ...
    fn absolute_x_addr(&self, addr: u16) -> IntValue<'ctx> {
        ...
        let x_ext = self
            .builder
            .build_int_z_extend(x, self.context.i16_type(), "x_ext");
        self.builder.build_int_add(
            x_ext,
            self.context.i16_type().const_int(addr as u64, false),
            "addr",
        )
    }
    ...
    fn absolute_y_addr(&self, addr: u16) -> IntValue<'ctx> {
        ...
        let y_ext = self
            .builder
            .build_int_z_extend(y, self.context.i16_type(), "y_ext");
        self.builder.build_int_add(
            y_ext,
            self.context.i16_type().const_int(addr as u64, false),
            "addr",
    ...
    fn concat(&self, low: IntValue<'ctx>, high: IntValue<'ctx>) -> IntValue<'ctx> {
        let low_ext = self
            .builder
            .build_int_z_extend(low, self.context.i16_type(), "low_ext");
        let high_ext = self
            .builder
            .build_int_z_extend(high, self.context.i16_type(), "high_ext");
        let high_shift = self.builder.build_left_shift(
            high_ext,
    ...
    fn indirect_x_addr(&self, offset: u8) -> IntValue<'ctx> {
        let x = self.builder.build_load(self.reg_x, "x").into_int_value();
        let addr_src = self.builder.build_int_add(
            x,
            self.context.i8_type().const_int(offset as u64, false),
            "addr_src",
        );
        let addr_low =
            self.builder
                .build_int_z_extend(addr_src, self.context.i16_type(), "addr_low");
        let low = self.read_mem(addr_low);
        let addr_high = self.builder.build_int_add(
            addr_low,
            self.context.i16_type().const_int(1, false),
            "addr_high",
        );
    ...
    fn zeropage_x_addr(&self, offset: u8) -> IntValue<'ctx> {
        let x = self.builder.build_load(self.reg_x, "x").into_int_value();
        let x_sum = self.builder.build_int_add(
            x,
            self.context.i8_type().const_int(offset as u64, false),
            "x_sum",
        );
        self.builder
            .build_int_z_extend(x_sum, self.context.i16_type(), "addr")
    }
    ...
    fn zeropage_y_addr(&self, offset: u8) -> IntValue<'ctx> {
        let y = self.builder.build_load(self.reg_y, "y").into_int_value();
        let y_sum = self.builder.build_int_add(
            y,
            self.context.i8_type().const_int(offset as u64, false),
            "y_sum",
        );
        self.builder
            .build_int_z_extend(y_sum, self.context.i16_type(), "addr")
    }
